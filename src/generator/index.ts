...
export class EventGenerator {
  ...
  fGenerateEvents(from: string | number | Date, to: string | number | Date) {
    const randomType = [...Object.keys(EventTypeT), this.faker.lorem.word(7)];
    const randomPlace = [
      'zoom',
      'telergam',
      'sdfgsdfgsdfgsd',
    ...
    for (let i = 0; i < this.fGetRandomNumber(10, 100); i++) {
      this.result.push({
        ...
        place: randomPlace[this.fGetRandomNumber(0, randomPlace.length)],
      });
    }
    fs.open('generating.json', 'w', (err) => {
      if (err) {
        console.log(err);
      }
    });
